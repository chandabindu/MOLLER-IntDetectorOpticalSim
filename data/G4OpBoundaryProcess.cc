...
{
  // G4cout << "In G4OpBoundaryProcess::PostStepDoI" << G4endl;
  ...
  if (OpticalSurface) {
    ...
    if (aMaterialPropertiesTable) {
      ...
      G4double tmpRefl = GetAngleDependentReflectivity(aMaterialPropertiesTable);
      if(tmpRefl){
      	// G4cout << "Setting Angle Dependent Reflectivity: " << tmpRefl << G4endl;
      	theReflectivity = tmpRefl; 
      }
...
G4double G4OpBoundaryProcess::GetAngleDependentReflectivity(G4MaterialPropertiesTable* propertyTable)
{
  G4double PdotN = OldMomentum * theGlobalNormal;
  G4double magP = OldMomentum.mag();
  G4double magN = theGlobalNormal.mag();
  G4double incidentangle = pi - std::acos(PdotN/(magP*magN));
  G4double IncAngle = incidentangle*180/pi;

  std::vector<G4String> names = propertyTable->GetMaterialPropertyNames();
  // std::string str;
  ...
  for(uint n = 0; n < names.size(); n++){
    if(names[n].contains("ANGLEREFLECTANCE")){
      angles.push_back(std::stof(&(names[n].data()[16])));
      Keys.push_back(names[n]);
      check = true;
      // G4cout << "Reflectance for: " << std::stof(&(names[n].data()[16])) << G4endl;
    }
  }
  if(!check) return 0;
