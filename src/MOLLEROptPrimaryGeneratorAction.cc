...
void MOLLEROptPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  //G4double sa = 45*pi/180;   Angular acceptance of the beam (how much it can deviate from the z-axis). This is now set from the macros
  G4double cosTheta = 1-(1-TMath::Cos(sa))*G4UniformRand(); G4double sinTheta = std::sqrt(1-cosTheta*cosTheta); G4double Phi = 2*pi*G4UniformRand();
